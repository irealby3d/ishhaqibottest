...
async function loadAdminData() {
    ...
    try {
        ...
        if(data.success){
            ...
            calculateTotal();
            renderAdminPage();
        } else showAdminError(data.error||'Server xatosi');
    } catch(e){
        console.error(e);
...
function applyFilters(){
    ...
    debounceTimer=setTimeout(()=>{
        ...
        filteredData=globalAdminData.filter(item=>{
            const mt=(item.name&&item.name.toLowerCase().includes(query))||(item.comment&&item.comment.toLowerCase().includes(query));
            const me=emp==='all'||item.name===emp;
            let mm=true,my=true;
            if(item.date){ const p=item.date.split('/');
...
function renderAdminPage(){
    if(!filteredData.length){
        document.getElementById('adminList').innerHTML=`<div class="empty-state"><div class="empty-icon">🔍</div><p>Ma'lumot topilmadi</p></div>`;
        document.getElementById('pagination').innerHTML='';
        return;
    }
    const start   =(currentPage-1)*ITEMS_PER_PAGE;
    const pageData=filteredData.slice(start,start+ITEMS_PER_PAGE);
    // Direktor ham ko'radi — faqat edit/delete yo'q
    const canEdit = myRole==='Boss'||myRole==='Admin';

    let html='';
    pageData.forEach(r=>{
        const isUsd=Number(r.amountUSD)>0;
        const rate =Number(r.rate)||0;
        // global index in filteredData for detail modal
        ...
        <div class="history-item" onclick="showAdminDetailModal(${gIdx})" style="cursor:pointer;">
            ...
            <div class="item-amounts">
                ...
                ${isUsd&&rate>0?`<span class="rate-tag">📈 ${rate.toLocaleString()}</span>`:''}
            </div>
            ${canEdit?`<div class="item-edit-hint">✏️ bosing →</div>`:''}
        </div>`;
    });
