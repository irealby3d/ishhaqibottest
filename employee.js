...
function initMyFilters() {
    ...
    // applyMyFilters sets myFilteredRecords
    applyMyFilters();
}
...
function renderMyPage() {
    ...
    pageData.forEach((r, i) => {
        ...
        // Compute effective rate if missing
        const effRate = rate > 0 ? rate : (usd > 0 && uzs > 0 ? Math.round(uzs / usd) : 0);
        // Original index in myFilteredRecords (not reversed)
...
function showMyDetailModal(idx) {
    ...
    if (!r) return;
    showDetailModal(r, false); // hodim o'zgartira olmaydi
